...
type SysMenu struct {
	...
	Path       string `gorm:"type:varchar(255);comment:路由路径(需符合Vue Router规范)" json:"path"`
	...
	Hidden     bool   `gorm:"type:tinyint(1);default:0;comment:是否隐藏(0显示 1隐藏)" json:"hidden"`
	KeepAlive  bool   `gorm:"type:tinyint(1);default:1;comment:是否缓存(0否 1是)" json:"keep_alive"`
	AlwaysShow bool   `gorm:"type:tinyint(1);default:0;comment:是否总是显示(即使只有一个子菜单)" json:"always_show"`
	Breadcrumb bool   `gorm:"type:tinyint(1);default:1;comment:是否显示面包屑" json:"breadcrumb"`
	Affix      bool   `gorm:"type:tinyint(1);default:0;comment:是否固定到标签页" json:"affix"`
