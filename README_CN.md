...
go:
  ...
  log:                          # 接口访问日志与微服务调用日志
    ...
    sink:                       # 接口日志异步总线（logsink）运行参数
      queue: 1024               # 每个 handler 的队列长度
      workers: 1                # 每个 handler 的消费协程数
      shutdown: 3000            # 退出时等待队列排空的毫秒数
    kafka:
      use: true                 # 是否启用 kafka 日志 handler（由 mgkafka 插件注册）
...
- **v2.0.5**：新增接口日志异步总线 `pkg/logsink`，接口日志可由外部插件（mgkafka、elasticsearch 插件等）注册 handler 接收；修复 swagger 路径为空时接口日志被全部跳过的缺陷；`SafeExit` 关闭组件前先排空日志队列。
...
- `mgin.SafeExit()` 会在关闭数据库连接（含 mongodb 插件）之前排空队列，减少退出时的日志丢失。
...
`logsink` 位于 `pkg/logsink`，只依赖 `pkg/models`，外部插件 import 它不会被迫拉入 gin / mongo 依赖。
...
func main() {
    app := mgin.NewApp("conf/application.yml", "myapp", Version, false)
...
注册时机：任意时刻均可，建议在 `mgin.NewApp()` 之后、`app.Run()` 之前；插件也可在自己的 `Init(ctx)` 里注册。
